...
export default class Pizza {
  ...
  getTotalCost() {
    let toppingCost = 0
    for (let code of this.toppingCodes) {
      toppingCost += TOPPINGS[code].cost
    }
    return this.getBaseCost() + toppingCost
  }

  getDescription() {
    let toppings = this.toppingCodes.map(code => TOPPINGS[code].name)
    let strNames = toppings.join(', ')
    return `A ${this.size} pizza with ${strNames}.`
  }
}
